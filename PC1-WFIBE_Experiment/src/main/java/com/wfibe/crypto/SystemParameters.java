...
public class SystemParameters {
     ...
     * 根据属性数量推荐向量维度
     */
    public static int recommendVectorDimension(int numAttributes) {
        // 基于生日悖论，确保碰撞概率 < 10^-9
        // n > k^2 * 10^9 / 2
        long required = (long) numAttributes * numAttributes * 1000000000L / 2;

        // 返回最接近的2的幂
        int dimension = 64;
        while (dimension < required && dimension < 8192) {
            dimension *= 2;
        }
     ...
     * 验证参数合理性
     */
    public static boolean validateParameters(int vectorDim, int attrCount) {
        if (vectorDim < 64 || vectorDim > 8192) {
            System.err.println("Vector dimension should be between 64 and 8192");
            return false;
        }

        if (attrCount < 1 || attrCount > 1000) {
            System.err.println("Attribute count should be between 1 and 1000");
            return false;
        }

        // 检查碰撞概率
        double collisionProb = (double)(attrCount * attrCount) / (2.0 * vectorDim);
        if (collisionProb > PerformanceBenchmarks.MAX_COLLISION_PROBABILITY) {
            System.err.println("Warning: Collision probability too high: " + collisionProb);
            System.err.println("Recommended dimension: " + recommendVectorDimension(attrCount));
        }
